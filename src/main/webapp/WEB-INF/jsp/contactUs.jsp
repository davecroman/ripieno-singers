<%@ include file="templates/tags.jspf" %>

<!doctype html>
<html>
<head>
    <title>The Ripieno Singers - Contact Us</title>
    <%@ include file="templates/head.jspf" %>
</head>

<body>
    <%@ include file="templates/header.jspf" %>

    <main>
       <div class="wrap">
           <div class="left-column with-margin-left-right">
            <h3 class="grey-text text-lighten-5">
                Visit Us
            </h3>
              <div class="text-block">
                <div>
                  <p> Rm 300 SSC Building, CFA Compound, 4427 Old Sta Mesa, Sta. Mesa, Manila 1016 </p>
                </div>
                 <div class="video-container responsive-video">
                   <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3860.966487088723!2d121.01108231484027!3d14.600984989802273!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3397c9de7e58e46d%3A0x8ef8d227ae46c96b!2s4427+Old+Sta.+Mesa+St%2C+Santa+Mesa%2C+Manila%2C+1016+Metro+Manila%2C+Philippines!5e0!3m2!1sen!2sau!4v1449389022985" width="600" height="450" frameborder="0" style="border:0" allowfullscreen></iframe>
                 </div>
               </div>
           </div>
           <div class="right-column with-margin-left-right">
               <div>
                    <h3 class="grey-text text-lighten-5">
                       Phone
                    </h3>
               </div>
               <div class="grey-text text-lighten-5 text-block">
                    <div><p> <i class="fa fa-mobile fa-1x white-font"></i> +63 2 897 1177 </p></div>
                    <div><p> <i class="fa fa-mobile fa-1x white-font"></i> +63 2 714-9618 </p></div>
               </div>
               <div class="grey-text text-lighten-5">
                   <h3 class="grey-text text-lighten-5">
                      Email
                   </h3>
               </div>
              <div class="grey-text text-lighten-5 text-block">
                   <p> <i class="fa fa-envelope-o fa-1x white-font"></i> theripienosingers@yahoo.com </p>
              </div>
              <div>
                <h3 class="grey-text text-lighten-5">
                    Connect
                </h3>
               </div>
               <div>
                 <div class="text-block">
                     <a class="with-side-padding-20" href="https://www.facebook.com/pages/The-Ripieno-Singers-Inc/752278521519093?fref=ts" target="_blank">
                         <i class="fa fa-facebook fa-2x white-font"></i>
                     </a>
                     <a class="with-side-padding-20" href="https://twitter.com/dripienosingers" target="_blank">
                         <i class="fa fa-twitter fa-2x white-font"></i>
                     </a>
                     <a class="with-side-padding-20" href="https://www.youtube.com/channel/UCbhMmSRV3YspiV33sbBHxvA" target="_blank">
                         <i class="fa fa-youtube fa-2x white-font"></i>
                     </a>
                     <a class="with-side-padding-20" href="https://instagram.com/theripienosingersinc/" target="_blank">
                         <i class="fa fa-instagram fa-2x white-font"></i>
                     </a>
                 </div>
               </div>

           </div>
       </div>
    </main>

    <%@ include file="templates/footer.jspf" %>
</body>

<%@ include file="templates/script.jspf" %>

</html>